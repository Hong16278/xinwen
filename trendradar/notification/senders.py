...
def send_to_feishu(
    ...
    *,
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        # 使用飞书交互式卡片 (Interactive Card)
        # 根据报告类型动态调整配色
        # 飞书支持的颜色: blue, wathet, turquoise, green, yellow, orange, red, carmine, violet, purple, indigo, grey
        color_map = {
            "当日汇总": "indigo",      # 沉稳，日报
            "当前榜单汇总": "blue",    # 标准，实时榜单
            "实时当前榜单": "blue",
        ...
        elements = [
            ...
            },
            {
                ...
                "elements": [
                    {
                        "tag": "plain_text",
                        "content": f"TrendRadar • {get_time_func().strftime('%Y-%m-%d %H:%M') if get_time_func else datetime.now().strftime('%Y-%m-%d %H:%M')}"
                    }
                ]
            }
        ]
        ...
        payload = {
            ...
            "card": {
                ...
                "header": {
                    "template": card_color,
                    "title": {
                        "content": f"TrendRadar 热点报告 ({i}/{len(batches)})" if len(batches) > 1 else "TrendRadar 热点报告",
                        "tag": "plain_text"
                    }
                },
                "elements": elements
